/**
 *Apr 24, 2013
 *3:40:35 PM
 *2013
 */
package com.civilizationcraft.CC.lib;

/**
 * Civilizationcraft
 * 
 * BlockIds.java
 * 
 * @author SuperClicheUsername
 * @license Lesser GNU Public License v3 (http://www.gnu.org/licenses/lgpl.html)
 * 
 */
public class BlockIds
    {

        public static int ORE_ALUMINUM_DEFUALT = 161;

        public static int ORE_ALUMINUM;
        
        public static int ORE_CHROMIUM_DEFUALT = 162;

        public static int ORE_CHROMIUM;
        
        public static int ORE_CINNABAR_DEFUALT = 163;

        public static int ORE_CINNABAR;
        
        public static int ORE_COBALT_DEFUALT = 164;

        public static int ORE_COBALT;
        
        public static int ORE_COPPER_DEFUALT = 165;

        public static int ORE_COPPER;
        
        public static int ORE_LEAD_DEFUALT = 166;

        public static int ORE_LEAD;
        
        public static int ORE_LITHIUM_DEFUALT = 167;

        public static int ORE_LITHIUM;
        
        public static int ORE_MAGNESIUM_DEFUALT = 168;

        public static int ORE_MAGNESIUM;
        
        public static int ORE_NICKEL_DEFUALT = 169;

        public static int ORE_NICKEL;
        
        public static int ORE_SILVER_DEFUALT = 170;

        public static int ORE_SILVER;
        
        public static int ORE_THORIUM_DEFUALT = 171;

        public static int ORE_THORIUM;
        
        public static int ORE_TIN_DEFUALT = 172;

        public static int ORE_TIN;
        
        public static int ORE_TITANIUM_DEFUALT = 173;

        public static int ORE_TITANIUM;
        
        public static int ORE_TUNGSTEN_DEFUALT = 174;

        public static int ORE_TUNGSTEN;
        
        public static int ORE_URANIUM_DEFUALT = 175;

        public static int ORE_URANIUM;
        
        public static int ORE_ZINC_DEFUALT = 176;

        public static int ORE_ZINC;

    }
